...
int main()
{
    ...
    double robot_y_offset = 7;//(67.42 / 2 )-30;
    ...
    std::string folder_path = "ds1//";
    std::string folder_look_up = "stitches_ds1.txt";
    ...
    int last_file_id = std::stoi(fs::path(image_files[0]).stem().string());
    ...
    for (size_t i = 1; i < image_files.size(); ++i)
    {
        ...
        if (pvo.processFrames(img_ref, img_next))
        {
            ...
            if (features.size() > 0)
            {
                ...
                int current_file_id = std::stoi(fs::path(image_files[i]).stem().string());
				...
				if (current_encoder_val - last_encoder_val > 2) //only perform BA if there's a significant movement (e.g., >2mm) to avoid noise
                {
					...
					ba.solve(features, dz_guess, 0.1, 0.12, 0.13, 0.11, robot_y_offset, encoder_val, 1); //non zero values for r1,r2,p,y to prevent algo getting stuck
                    ...
                    for (int r = 0; r <= num_rings; ++r) {
                        ...
                        for (int p = 0; p <= num_points; ++p) {
                            ...
                            Eigen::Vector3d pt_cam = R_pipe * pt_pipe + Eigen::Vector3d(x_off, robot_y_offset, 0);
                    ...
                    std::string output_dir = "centralisation_results";
